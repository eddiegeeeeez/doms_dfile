...
namespace DFile.backend.Controllers
{
    ...
    public class DepartmentsController : TenantAwareController
    {
        ...
        public async Task<ActionResult<IEnumerable<DepartmentResponseDto>>> GetDepartments([FromQuery] bool showArchived = false, [FromQuery] bool hierarchy = false)
        {
            ...
            if (hierarchy)
            {
                // Build hierarchy in-memory from the single flat query (no N+1)
                var rootDepts = departments.Where(d => d.ParentDepartmentId == null).ToList();
                var result = rootDepts.Select(d => BuildHierarchy(d, departments, parentNames, userNames)).ToList();
                return Ok(result);
            }

            return Ok(departments.Select(d => MapToDepartmentDto(d, parentNames, userNames)).ToList());
        }

        private static DepartmentResponseDto BuildHierarchy(Department dept, List<Department> allDepts, Dictionary<string, string> parentNames, Dictionary<int, string> userNames)
        {
            var children = allDepts
                .Where(d => d.ParentDepartmentId == dept.Id)
                .Select(d => BuildHierarchy(d, allDepts, parentNames, userNames))
                .ToList();

            var dto = MapToDepartmentDto(dept, parentNames, userNames);
            dto.Children = children.Count > 0 ? children : null;
            return dto;
        }

        private static DepartmentResponseDto MapToDepartmentDto(Department d, Dictionary<string, string> parentNames, Dictionary<int, string> userNames)
        {
