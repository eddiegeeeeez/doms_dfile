...
// CORS — permissive in Development; restricted origins in Production
builder.Services.AddCors(options =>
{
    if (builder.Environment.IsDevelopment())
    {
        options.AddPolicy("DFileCors", policy =>
            policy.AllowAnyOrigin().AllowAnyMethod().AllowAnyHeader());
    }
    else
    {
        var configured = builder.Configuration["AllowedOrigins:Frontend"];
