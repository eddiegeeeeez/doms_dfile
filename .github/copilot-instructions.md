...
```powershell
cd DFile.frontend ; npm run build  # next build + postbuild robocopy → DFile.backend/wwwroot/
```
`robocopy` is Windows-only. For Docker use the root `Dockerfile` instead.
