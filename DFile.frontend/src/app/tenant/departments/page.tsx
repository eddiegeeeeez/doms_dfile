...
import { Building2, Plus, Search, Pencil, Archive, RotateCcw } from "lucide-react";
            ...
            <Card>
                <CardHeader>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <CardTitle>{showArchived ? "Archived Departments" : "Departments"}</CardTitle>
                            <CardDescription>{filtered.length} departments</CardDescription>
                        </div>
                                ...
                                <TableBody>
                                    {filtered.map(d => (
                                        <TableRow key={d.id}>
                                            ...
                                            <TableCell><Badge variant={d.status === "Active" ? "success" : "muted"}>{d.status}</Badge></TableCell>
                                            <TableCell>
                                                <div className="flex gap-1">
                                                    <Button variant="ghost" size="icon" onClick={() => openEdit(d)}><Pencil className="h-4 w-4" /></Button>
                                                    {d.status === "Active" ? (
                                                        <Button variant="ghost" size="icon" onClick={() => setArchiveTarget(d.id)}><Archive className="h-4 w-4" /></Button>
                                                    ) : (
                                                        <Button variant="ghost" size="icon" onClick={() => restoreMutation.mutateAsync(d.id)}><RotateCcw className="h-4 w-4" /></Button>
                                                    )}
                                                </div>
                                            </TableCell>
                                        </TableRow>
