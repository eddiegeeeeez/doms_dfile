...
function Breadcrumb({ pathname, navSections, homePath }: { pathname: string; navSections: NavSection[]; homePath: string }) {
    ...
    return (
        <nav className="flex items-center gap-2 text-sm">
            {home && (
                <>
                    <Link href={homePath} className="text-muted-foreground hover:text-foreground transition-colors font-medium">
                        {home.label}
                    </Link>
        ...
        <div className="flex flex-col h-full">
            ...
            <div className={`flex items-center shrink-0 border-b border-border/60 ${isCollapsed ? "h-16 justify-center px-2" : "h-16 px-5"}`}>
                {!isCollapsed ? (
                    ...
                    <Link href={homePath} className="flex items-center justify-center w-10 h-10 rounded-xl bg-primary/10">
        ...
        <TooltipProvider delayDuration={100}>
            <div className="min-h-screen bg-background flex">
                ...
                <div className={`flex-1 flex flex-col min-w-0 min-h-screen transition-[margin] duration-300 ease-out
                    ...
                    {/* Header */}
                    <header className="h-[72px] bg-background/80 backdrop-blur-md border-b border-border/50 flex items-center px-6 sm:px-8 gap-4 sticky top-0 z-30">
                        {/* Mobile: open drawer */}
                        <Button
                            variant="ghost"
                            size="icon"
                    ...
                    {/* Page Content */}
                    <main className="flex-1 px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[1400px] mx-auto">
                        {children}
                    </main>
                </div>
            </div>
